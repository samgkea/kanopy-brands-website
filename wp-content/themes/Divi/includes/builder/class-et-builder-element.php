...
class ET_Builder_Element {
	...
	*/
	...
	static function get_child_slugs( $post_type, $include_additional_shortcode_slug = false ) {
		$child_slugs = array();
		...
		if ( ! empty( $child_modules ) ) {
			foreach( $child_modules as $module ) {
				if ( ! empty( $module->child_slug ) ) {
					...
					if ( $include_additional_shortcode_slug && ! empty( $module->additional_shortcode ) ) {
						$child_slugs[ $module->additional_shortcode ] = $module->additional_shortcode;
					}

					if ( $include_additional_shortcode_slug && ! empty( $module->additional_shortcode_slugs ) ) {
						foreach ( $module->additional_shortcode_slugs as $additional_shortcode_slug ) {
							$child_slugs[ $additional_shortcode_slug ] = $additional_shortcode_slug;
						}
					}
				}
			}
	...
	static function get_structure_module_slugs( $include_additional_shortcode_slug = false ) {
		...
		foreach( $structure_modules as $structural_module ) {
			...
			if ( $include_additional_shortcode_slug && ! empty( $structural_module->additional_shortcode ) ) {
				self::$structure_module_slugs[] = $structural_module->additional_shortcode;
			}

			if ( $include_additional_shortcode_slug && ! empty( $structural_module->additional_shortcode_slugs ) ) {
				foreach ( $structural_module->additional_shortcode_slugs as $additional_shortcode_slug ) {
					self::$structure_module_slugs[] = $additional_shortcode_slug;
				}
			}
		}
